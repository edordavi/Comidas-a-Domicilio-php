...
require_once '../conexionComidas.php';
    ...
    function insertaRecetasPizza(){
        ...
        $consulta=$db->prepare("CALL sp_nuevo_recetaspizza(:articulo,:ingrediente);");
    ...
    function actualizaRecetasPizza(){
        global $server,$bd,$user,$password;
        ...
        if (isset($_GET["articulo2"])){
            ...
            $consulta=$db->prepare("CALL sp_actualiza_recetaspizza(:articulo1,:articulo2,:ingrediente1,:ingrediente2);");
            ...
            consultaRecetasPizzaActualizar($_GET['articulo'],$_GET['ingrediente']);
        }
    }
    
    function eliminaRecetasPizza(){
        global $server,$bd,$user,$password;
        ...
        $articulo = $_GET["articulo"];
		$ingrediente = $_GET["ingrediente"];
        $consulta=$db->prepare("CALL sp_elimina_recetaspizza(:articulo,:ingrediente);");
    ...
    function consultaRecetasPizzaActualizar($articulo1,$ingrediente1){
        global $server,$bd,$user,$password;
        $db = new PDO("mysql:host=".$server.";dbname=".$bd.";charset=utf8",$user, $password);
        ...
        $consulta=$db->prepare("CALL sp_get_recetaspizza_esp(:articulo,:ingrediente);");
    ...
    function consultaRecetasPizza(){
        ...
        $consulta=$db->prepare("CALL sp_get_recetaspizza();");
        $consulta->execute();
        $filas=$consulta->fetchAll(PDO::FETCH_ASSOC);
        tabla($filas);
        $db = null;
        ...
        foreach ($filas as $fila){
            ...
            echo <<<HTML
                <td><input type='button' onclick='consulta(
                    "cls_recetaspizza.php?accion=eliminar&articulo={$fila["ARTICULO"]}&ingrediente={$fila["INGREDIENTE"]}");' value='ELIMINAR'>
                </td> 
                <td><input type='button' 
                    onclick='consulta("cls_recetaspizza.php?accion=actualizar&articulo={$fila["ARTICULO"]}&ingrediente={$fila["INGREDIENTE"]}");'
                        value="ACTUALIZAR"></td>
HTML;
            echo "</tr>";
        }
		...
		$db = null;
			...
			echo "</select></td>";	
                ...
                ?>
                <td colspan="2"><input type="button" value="Agregar" onclick="insertaRecetasPizza();"></td>
