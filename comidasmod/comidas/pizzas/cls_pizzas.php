...
require_once '../conexionComidas.php';
    ...
    function insertaFamilia(){
        ...
        $tam = $_GET["tam"];
        $prc = $_GET["prc"];
        $consulta=$db->prepare("CALL sp_nueva_pizza(:art,:tam,:prc);");
    ...
    function eliminaFamilia(){
        ...
        $art = $_GET["art"];
        $tam = $_GET["tam"];
        $consulta=$db->prepare("CALL sp_elimina_pizza(:art,:tam);");
        $consulta->bindParam(":art",$art, PDO::PARAM_STR, 10);
        $consulta->bindParam(":tam",$tam, PDO::PARAM_STR, 1);
        $consulta->execute();
    ...
    function consultaFamilias($art,$tam){
        ...
        $consulta=$db->prepare("CALL sp_get_pizzas(:art,:tam);");
        $consulta->bindParam(":art",$art, PDO::PARAM_STR, 10);
        $consulta->bindParam(":tam",$tam, PDO::PARAM_STR, 1);
        $consulta->execute();
//        print_r($consulta->errorInfo());
        ...
        foreach ($filas as $fila){
            ...
            echo <<<HTML
                <td><input type='button' onclick='consulta(
                    "cls_pizzas.php?accion=eliminar&art={$fila["ARTICULO"]}&tam={$fila["TAMANIO"]}");' value='ELIMINAR'>
                </td> 
HTML;
            echo "</tr>";
        }
		...
		$db = null;
			...
			echo "</select></td>";	
                ...
                <td><input type="number" name="prc" id="prc" class="textEntry" 
                           placeholder="Precio de Ingrediente" maxlength="3" min="0" max="999" step="10"></td>
